...
frappe.ui.form.on('Employee Statements', {
	...
	send_customer_statements: function (frm) {
	  ...
	  frappe.confirm(
		...
		function () {
		  frappe.call({
			method: "anfac_retail.api.api.statements_sender_scheduler",
			args: {
			  manual: true
			},
			callback: function (r) {
	...
	preview: function (frm) {
	  if (frm.doc.employee != undefined && frm.doc.employee != "") {
		frappe.call({
		  method: "anfac_retail.api.api.get_report_content_3",
		  args: {
	...
	send_email: function (frm) {
	  if (frm.doc.customer != undefined && frm.doc.customer != "") {
		frappe.call({
		  method: "anfac_retail.api.api.send_individual_statement",
		  args: {
			company: frm.doc.company,
