...
def get_data(filters):
	...
	for acco in openings:
		...
		if not is_group :
			...
			if acco.account in filters.account:
				incash.append(
					{
						"type" : "Opening",
						"customer" : acco.account,
...
def get_journal(filters):
	...
	c.debit as debit,
	 ...
	 on p.name = c.parent
	 where p.posting_date between "{_from}" and "{to}" and p.docstatus = 1  and c.account_type  = "Cash" and c.debit > 0;
	""" ,  as_dict = 1)
	return data
...
def get_payments(filters):
	...
	from `tabPayment Entry` 
	where posting_date between "{_from}" and "{to}" and payment_type = "Pay"  and docstatus = 1
	;""" , as_dict = 1)
	return data
...
def get_journal_payment(filters):
	...
	c.debit as debit,
	 ...
	 left join `tabJournal Entry` p 
	 on p.name = c.parent
	 where p.posting_date between "{_from}" and "{to}" and p.docstatus = 1  and c.account_type  = "Cash" and c.credit > 0;
	""" ,  as_dict = 1)
	return data
